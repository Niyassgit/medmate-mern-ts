...
export class DoctorPendingConnectionsUseCase implements IDoctorPendingConnectionsUseCase{
    ...
    async execute(userId: string): Promise<ConnectionsListOnModalDTO[]> {
        ...
        if(!doctorId) throw new UnautharizedError(ErrorMessages.UNAUTHERIZED_SOCKET);
        const pendingConnections=await this._connectionRepository.pendingConnectionForDoctor(doctorId);
        if(!pendingConnections) return [];
        const mappedRep=ConnectionMappers.toConnectionListModal(pendingConnections,this._storageService);
        return mappedRep;
    }
}